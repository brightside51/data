...
class DataReader(Dataset):
    ...
    def getsubj(self, idx: int = 0 or str, save: bool = False):
        ...
        if self.args.data_format == 'nii':
            ct_data = nib.load(f"{self.args.data_fp}/ct/{idx}").get_fdata()
            mask_data = nib.load(f"{self.args.data_fp}/mask/{idx}").get_fdata()

            # Slice Image Pre-Processing | HU Clipping, flipping, Rescaling & Resizing
            idx_hflip = (torch.rand(1) < (self.args.h_flip / 100)); ct, mask = [], []
            for s in range(ct_data.shape[-1]):
