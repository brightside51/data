   ...
   # Single Batch Fetching
    def __getitem__(self, idx: int = 0 or str, save: bool = False):
        ...
        if self.args.data_format == 'npy':
            ...
            img_data = torch.empty((full_data.shape[0], self.args.img_size, self.args.img_size))
            ...
            if self.args.data_prep:
                ...
                for s in range(full_data.shape[0]):
                    slice_data = Image.fromarray(full_data[s]).resize(( self.args.img_size,
                                                                        self.args.img_size))
                    if idx_hflip: slice_data = self.h_flip(slice_data)
                    img_data[s, :, :] = torch.Tensor(self.transform(slice_data))
            ...
            if self.args.num_slice == 0:
                self.args.num_slice = full_data.shape[0]
            else:
